---
title: Experiments with Go & Ruby
...
because I needed learn some Ruby for a task anyways.
...
This makes it super quick to get certain tasks done in Ruby.
...
I find myself spending way less time googling for API documentation.


### What Go taught me

__Static typing__ speeds up the workflow since I see many errors right in the editor.
It gives me some security that I didn't have any silly typo errors.
Go's strictness also, by some extend, enforces cleaner code by not allowing me to declare unused variables and so on.
...
I need to think more about their relations and about how to organize them.
...
Writing Go took me a little longer since I had to implement a lot of functionality already available in Ruby.
However, it also made me __think about what the computer actually needs to do__ for a certain task.
This way my program ended up doing less unnecessary things. It might also be more performant code.

Looking at the one-liner implementation of `most_used` in Ruby,
the Go version is more difficult to understand.
However, if you understand the Go version you actually know what the computer is going to do:
...
I love to see the web as a programming platform improving in areas
...
The latest efforts enables us to reason about problems in [simple and declarative ways](https://github.com/staltz/todomvc-cycle).

Now I would love see a way to bring the safety and robustness of Go to the web.
Unfortunately, I'm not really convinced by [the solutions](http://www.typescriptlang.org/) [out there](http://flowtype.org/).
Maybe we could achieve the same goals in a dynamic language by advancing your testing technologies?
